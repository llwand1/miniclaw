 ...
 *
 * 威胁模型：MiniClaw 的 office-server 监听 127.0.0.1:18791，
 * 用户在本机浏览器访问恶意网站时，该网站可尝试 fetch('http://127.0.0.1:18791/api/...')。
 * 浏览器同源策略会阻止读取响应，但「简单请求」(GET/POST 表单) 仍可发出，
 * 可能触发 AI 对话、文件写入等副作用。
 *
 * 防御：
 * 1) 校验 Origin/Referer 头，只允许 localhost / 127.0.0.1 / file://。
...
/** 允许的 Origin 协议。file:// 用于 Electron 加载本地文件场景。 */
function isAllowedOrigin(origin: string | undefined): boolean {
  if (!origin) return false;
  // file:// 协议（Electron 本地资源）
  if (origin === 'file://' || origin.startsWith('file://')) return true;
  // capacitor:// / ionic:// 等混合协议（预留）
...
export function originCheck(req: Request, res: Response, next: NextFunction): void {
  ...
  // GET 请求若不带 Origin/Referer（如 curl、Electron 内部 fetch）则放行，
  // 但若有 Origin 必须是允许的来源。
...
export const corsWhitelist = (origin: string | undefined, callback: (err: Error | null, allow?: boolean) => void): void => {
  // 允许同源（无 Origin，如服务端到服务端、Electron 内部）
