...
export function Toggle({ checked, onChange, disabled }: { checked: boolean; onChange: (v: boolean) => void; disabled?: boolean }) {
  return (
    <label style={{ position: 'relative', display: 'inline-block', width: 44, height: 24, cursor: disabled ? 'not-allowed' : 'pointer', opacity: disabled ? 0.5 : 1, transition: 'transform .16s cubic-bezier(.2,.7,.3,1)' }}
      ...
      <span style={{ position: 'absolute', inset: 0, background: checked ? 'var(--accent)' : '#d1d5db', borderRadius: 24, transition: '0.3s', boxShadow: checked ? '0 2px 8px rgba(0,185,107,.3)' : 'none' }}>
